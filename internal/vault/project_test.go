...
func TestWriteProjectFixture(t *testing.T) {
	...
	// The v2 hybrid identity and the same DEK wrapped to it. Sharing one DEK
	// between both wrappings lets the other clients prove they open to the same
	// 32 bytes whichever version they are handed.
	hybridPub, hybridPriv, err := UpgradeIdentity(pub, priv)
	if err != nil {
		t.Fatalf("UpgradeIdentity: %v", err)
	}
	hybridWrapped, err := WrapProjectDEK(dek, hybridPub)
	if err != nil {
		t.Fatalf("WrapProjectDEK (hybrid): %v", err)
	}
	...
	fixture := map[string]string{
		...
		"hybridPublicKeyBase64":  b64(hybridPub),
		"hybridPrivateKeyBase64": b64(hybridPriv),
		"hybridWrappedDekBase64": b64(hybridWrapped),
